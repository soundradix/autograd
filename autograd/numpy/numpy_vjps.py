...
defvjp(
    anp.power,
    lambda ans, x, y: unbroadcast_f(x, lambda g: g * y * x ** anp.where(x, anp.where(y, y - 1, 1.0), 1.0)),
...
defvjp(
    anp.tanh, lambda ans, x: lambda g: anp.where(anp.abs(x) < 70, g / anp.cosh(anp.clip(x, -70, 70)) ** 2, 0)
)
